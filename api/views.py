...
#                         order.save()
#                         api_models.Notification.objects.create(user=order.student, order=order,
#                                                                type="Course Enrollment Completed")
#
#                         for o in order_items:
#                             # Check if the user is already enrolled in the course
#                             if not api_models.EnrolledCourse.objects.filter(
#                                 course=o.course,
#                                 user=order.student
#                             ).exists():
...
class SearchCourseAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # learn lms
        return api_models.Course.objects.filter(title__icontains=query, platform_status="Published",
    ...
    def perform_create(self, serializer):
        ...
        for key, value in self.request.data.items():
            if key.startswith('variant') and '[variant_title]' in key:
                ...
                print("FILES being added:", self.request.FILES)
