...
def custom_slugify(value):
    return translit_slugify(value)

def generate_unique_slug(instance, field_value):
    base_slug = custom_slugify(field_value)
    slug = base_slug
    num = 1
...
class Category(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:  # Якщо slug порожній або None
...
class Course(models.Model):
    ...
    language = models.CharField(choices=LANGUAGE, default="Українська", max_length=100)
    level = models.CharField(choices=LEVEL, default="Початківець", max_length=100)
